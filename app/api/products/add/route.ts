...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!newBlock) {
      return NextResponse.json({ error: 'Missing newBlock' }, { status: 400 })
    }
    ...
    // Insert new block before the closing ] of the array
    const lastBracket = current.lastIndexOf(']')
    if (lastBracket === -1) {
      return NextResponse.json({ error: 'Could not find closing ] in products.ts' }, { status: 500 })
    }

    const updated =
      current.slice(0, lastBracket) +
      newBlock.trimEnd() + '\n' +
      current.slice(lastBracket)
    ...
    const commitRes = await fetch(
      `${GITHUB_API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${FILE_PATH}`,
      {
        ...
        headers,
        body: JSON.stringify({
          message: 'Add new product via admin dashboard',
